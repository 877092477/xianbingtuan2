        ...
        if ( [data length] > 0 && !error ) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if(![versionsInAppStore count]) {
                    ...
                    NSLog(@"%@",OLDVERSION);
                    if ([FNCurrentVersion isEqualToString:Setting_checkVersion] || [QJCheckVersionUpdate versionlessthan:[GetUserDefaut isKindOfClass:[NSString class]] && GetUserDefaut ? GetUserDefaut : OLDVERSION Newer:currentAppStoreVersion])
                    {
                        NSLog(@"暂不更新");
                    }else{
                        NSLog(@"请到appstore更新%@版本",currentAppStoreVersion);
...
-(NSString *)getAppStroeUrlForiPhone{
    return [NSString stringWithFormat:@"https://itunes.apple.com/cn/app/chao-ju-hui-gou-suo-xu-de-fan-li/%@?mt=8",APPID];
}
