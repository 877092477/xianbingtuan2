...
- (void)setFontSize:(CGFloat)fontSize {
    for (FDSlideBarItem *item in _items) {
        [item setItemTitleFont:fontSize];
    }
}

- (void)setSelectedfontSize:(CGFloat)SelectedfontSize{
    for (FDSlideBarItem *item in _items) {
        [item setItemSelectedTileFont:SelectedfontSize];
    }
}
...
- (void)setupItems {
    ...
    for (NSString *title in _itemsTitle) {
        ...
        if(self.is_bubig==YES){
            itemW=self.frame.size.width/3;
        }
        if(self.is_mean){
    ...
    if(self.is_mean){
        NSString *textLeng=[_itemsTitle firstObject];
        sliderWith=textLeng.length*14+30;
        _sliderView.frame = CGRectMake(firstItem.frame.size.width/2-sliderWith/2, self.frame.size.height - SLIDER_VIEW_HEIGHT, sliderWith, SLIDER_VIEW_HEIGHT);
        
    }
    if (self.is_middle==YES) {
        FDSlideBarItem *lastItem = [self.items lastObject];
        if (CGRectGetMaxX(lastItem.frame)<self.frame.size.width) {
            _scrollView.width=CGRectGetMaxX(lastItem.frame);
            _scrollView.x=(self.frame.size.width-CGRectGetMaxX(lastItem.frame))/2;
        }
    }
