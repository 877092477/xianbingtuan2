...
- (void)takePhoto: (UITapGestureRecognizer*)gesture {
    NSLog(@"拍照");
    [self.recorderManager takePhoto:^(BOOL success, UIImage *image) {
...
- (void)showImage: (UIImage*)image {
    ...
    CGFloat height = XYScreenWidth * image.size.height / image.size.width;
    UIView *vImage = [[UIView alloc] initWithFrame:CGRectMake(0, 0, XYScreenWidth, XYScreenHeight)];
    NSLog(@"%f  %f", image.size.width, image.size.height);
    
    vImage.backgroundColor = UIColor.blackColor;
    [_contentView addSubview:vImage];
    UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, XYScreenWidth, height)];
    imgView.image = image;
    imgView.center = vImage.center;
    imgView.contentMode = UIViewContentModeScaleToFill;
    [vImage addSubview:imgView];
    
    CGFloat deta = [UIScreen mainScreen].bounds.size.width/375.0;
    CGFloat width = 65.0*deta;

    UIButton *cancelButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [cancelButton setBackgroundImage:[UIImage imageNamed:@"record_video_cancel"] forState:UIControlStateNormal];
    cancelButton.frame = CGRectMake((vImage.width - width)/2, vImage.height - 130*deta, width, width);

    UIButton *confirmButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [confirmButton setBackgroundImage:[UIImage imageNamed:@"record_video_confirm"] forState:UIControlStateNormal];
    confirmButton.frame = CGRectMake((vImage.width - width)/2, cancelButton.frame.origin.y , width, width);

    [vImage addSubview:confirmButton];
    [vImage addSubview:cancelButton];
    __weak typeof(self) instance = self;
    [cancelButton addJXTouch:^{
        [instance clickCancel];
        [vImage removeFromSuperview];
    }];
    [confirmButton addJXTouch:^{
        if (instance.completionImageBlock && image) {
            instance.completionImageBlock(image);
        }
        [vImage removeFromSuperview];
        [instance dismiss:NO];
    }];
    ...
    CGRect confirmRect = confirmButton.frame;
