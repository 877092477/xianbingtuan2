...
@property (nonatomic, strong) AVCaptureStillImageOutput *imageFileOutput;
...
- (instancetype)init{
    ...
    if (self) {
        ...
        self.imageFileOutput = [[AVCaptureStillImageOutput alloc]init];
        NSDictionary *outputSettings = @{AVVideoCodecKey:AVVideoCodecJPEG};
        [self.imageFileOutput setOutputSettings:outputSettings];
...
- (void)prepareForRecord{
    ...
    [self.captureSession canAddOutput:self.imageFileOutput] ? [self.captureSession addOutput:self.imageFileOutput] : nil;
...
- (void)takePhoto:(void(^)(BOOL success, UIImage* image))complete {
    AVCaptureConnection *captureConnection=[self.imageFileOutput connectionWithMediaType:AVMediaTypeVideo];
    //根据连接取得设备输出的数据
    [self.imageFileOutput captureStillImageAsynchronouslyFromConnection:captureConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
