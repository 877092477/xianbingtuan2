...
@interface AAChartView()<WKUIDelegate, WKNavigationDelegate, UIWebViewDelegate,UIScrollViewDelegate> {
...
- (void)setUpBasicWebView {
    ...
    [notiCenter addObserver:self selector:@selector(receiveNotification:) name:@"movementFate" object:nil];
...
{
    NSLog(@"Fate=== %@ === %@" ,noti.userInfo, noti.name);
    NSDictionary *dic=noti.userInfo;
    NSInteger move=[dic[@"Fate"] integerValue];
    if(move<7||move==7){
        move=0;
    }
    if(move==14){
    ...
    if (AASYSTEM_VERSION >= 9.0) {
        scrollView = _wkWebView.scrollView;
    } else {
        scrollView = _uiWebView.scrollView;
    }
    [scrollView setContentOffset:CGPointMake(move*46,0) animated:NO];
    ...
    if (AASYSTEM_VERSION >= 9.0) {
        [_wkWebView  evaluateJavaScript:functionNameStr completionHandler:^(id item, NSError * _Nullable error) {
            if (error) {
                ...
                AADetailLog(@"!!!!!WARNING!!!!! THERE ARE SOME ERROR INFOMATION_______%@",errorDic);
            }
        }];
    } else {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    CGFloat scrollContentSizeWidth = scrollView.contentSize.width;
    CGFloat scrollOffset = scrollView.contentOffset.x;
    if(scrollOffset ==0) {
        // 滚动到了左边
        //XYLog(@"拉取一下");
        if ([UserAccessToken kr_isNotEmpty]) {
            [FNNotificationCenter postNotificationName:@"refreshStatistics" object:nil];
        }
    }
...
- (void)setScrollEnabled:(BOOL)scrollEnabled {
    ...
    _uiWebView.scrollView.showsVerticalScrollIndicator=NO;
    _uiWebView.scrollView.showsHorizontalScrollIndicator=NO;
