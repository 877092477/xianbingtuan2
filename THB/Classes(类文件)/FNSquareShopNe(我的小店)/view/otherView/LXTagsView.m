...
//        UIButton *button =[UIButton LXButtonNoFrameWithTitle:_dataA[i] titleFont:self.tagFont Image:nil backgroundImage:nil backgroundColor:[UIColor hexStringToColor:@"3344FF"] titleColor:[UIColor whiteColor]];
        ...
        button.borderColor = FNGlobalTextGrayColor;
        [button setImage:nil forState:UIControlStateNormal];
        [button setBackgroundImage:nil forState:UIControlStateNormal];
        [button setTitle:_dataA[i] forState:UIControlStateNormal];
...
-(void)addBtnClick:(UIButton*)btn{
    if (self.tagClick) {
        self.tagClick(self.dataA[btn.tag]);
    }
}
-(void)layoutItems{
    CGFloat viewVMargin = 2.5;// self.viewVMargin;//整体上间距
    ...
    CGFloat allWidth = FNDeviceWidth-120;//总宽度
