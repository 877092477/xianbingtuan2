//
//  FNStoreGoodsAttriManagerController.m
//  新版嗨如意
//
...
@interface FNStoreGoodsAttriManagerController ()<FNStoreGoodsSpecHeaderViewDelegate, UITableViewDelegate, UITableViewDataSource, FNStoreManagerGoodsFooterViewDelegate, FNStoreGoodsSpecAddCellDelegate, FNStoreGoodsAttriAddCellDelegate>
...
@property (nonatomic, strong) FNStoreGoodsAttriAddCell *headerCell;
...
@implementation FNStoreGoodsAttriManagerController
...
//    _headerCell = [self.tbvSpec dequeueReusableCellWithIdentifier:@"FNStoreGoodsAttriAddCell"];
    _headerCell = [[FNStoreGoodsAttriAddCell alloc] init];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (section == 0) {
        ...
        FNStoreGoodsAttriHeaderView* headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:@"FNStoreGoodsAttriHeaderView"];
        if (headerView == nil) {
            headerView = [[FNStoreGoodsAttriHeaderView alloc]initWithReuseIdentifier:@"FNStoreGoodsAttriHeaderView"];
        }
...
//        [FNTipsView showTips:@"请选择商品属性"];
        if ([_delegate respondsToSelector:@selector(goodsAttri:didSelected:)]) {
            [_delegate goodsAttri:self didSelected: nil];
        }
        return;
    }
    
    NSMutableArray *specs = [[NSMutableArray alloc] init];
    for (FNStoreGoodsSpecManagerModel *spec in _selectAttris) {
        
        FNStoreGoodsSpecManagerModel *ans = [[FNStoreGoodsSpecManagerModel alloc] init];
        ans.id = spec.id;
        ans.name = spec.name;
        ans.type = spec.type;
        
        NSMutableArray *tags = [[NSMutableArray alloc] init];
        for (FNStoreGoodsSpecDataModel *tag in spec.list) {
            if (tag.isSelected) {
                [tags addObject: tag];
            }
        }
        ...
        if (tags.count < 2) {
            if ([spec.name kr_isNotEmpty]) {
                [FNTipsView showTips:[NSString stringWithFormat:@"请添加属性名字"]];
                return;
            }
            [FNTipsView showTips:[NSString stringWithFormat:@"请选择至少两项【%@】属性", spec.name]];
            return;
        }
    ...
    if ([_delegate respondsToSelector:@selector(goodsAttri:didSelected:)]) {
        [_delegate goodsAttri:self didSelected: specs];
    }
}
...
- (void)requestSpecs{
    ...
    [FNRequestTool requestWithParams:params api:@"mod=appapi&act=rebate_goods_specs&ctrl=spesc_detail" respondType:(ResponseTypeArray) modelType:@"FNStoreGoodsSpecManagerModel" success:^(id respondsObject) {
        ...
        NSMutableArray *array = [[NSMutableArray alloc] init];
        for (FNStoreGoodsSpecManagerModel* spec in _specs) {
            [array addObject:spec.name];
        }
        [self.headerCell setTitles: array];
        
        if (self.baseSpecs && self.baseSpecs.count > 0) {
            
            [self.selectAttris addObjectsFromArray: self.baseSpecs];
        }
