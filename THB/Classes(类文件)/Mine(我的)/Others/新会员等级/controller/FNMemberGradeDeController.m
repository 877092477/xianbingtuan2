//
//  FNMemberGradeDeController.m
//  THB
//
//  Created by Jimmy on 2019/1/15.
...
@interface FNMemberGradeDeController ()
...
@implementation FNMemberGradeDeController
...
- (void)apiRequstClassify{
    ...
    [FNRequestTool requestWithParams:params api:@"mod=appapi&act=connection&ctrl=mem_cate" respondType:(ResponseTypeNone) modelType:@"" success:^(id respondsObject) {
        ...
        [selfWeak setTopCate];
        [selfWeak.titleScrollBg setNoPlaceholderUrlImg:dictry[@"bg_img"]];
...
- (void)setupChildVc
{
    if (_ClassifyArr.count>0) {
        for (int i = 0 ; i<_ClassifyArr.count; i++) {
            ...
            FNGradeItemDeController *VC = [[FNGradeItemDeController alloc] init];
            VC.title = model.str;
            VC.type = model.type;
