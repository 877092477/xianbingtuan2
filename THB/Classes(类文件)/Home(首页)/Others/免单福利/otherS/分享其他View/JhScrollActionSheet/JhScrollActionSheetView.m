...
/** 两个滚动视图的总高度 */
#define kDescribeViewFrameH (self.isHiddenDescribeLabel == YES ? 0 : 50)
/** 两个滚动视图的总高度 */
#define kScrollViewFrameH (self.isHiddenOtherCollectionView == YES ? 100 : 200)
/** 整个视图的总高度 */
#define kViewFrameH  (kTitleViewFrameH + kDescribeViewFrameH + kScrollViewFrameH + 30 )
//+ kBottomSafeHeight
...
/** describeLabel传nil时,为YES */
@property (assign, nonatomic) BOOL isHiddenDescribeLabel;
/**
 * 收起视图
 */
- (void)dismiss;
...
-(void)setUpSubviews{
    ...
    if (!_describeStr.length) {
        _isHiddenDescribeLabel = YES;
    }else{
        _isHiddenDescribeLabel =NO;
    }
    ...
    if (!_otherArray.count) {
        ...
        UIView *line=[[UIView alloc]initWithFrame:CGRectMake(10,CGRectGetMaxY(self.titleLabel.frame), Kwidth-20, 1)];
        line.backgroundColor=JhColor(243, 243, 243);
        [self.actionSheetView addSubview:line];
        line.hidden=_isHiddenTitleLabel;
    ...
    _actionSheetView.backgroundColor = [UIColor whiteColor];// JhColor(240, 240, 240);
...
-(UIView *)actionSheetView{
    if (!_actionSheetView) {
        ...
        actionSheetView.cornerRadius=5;
...
-(UILabel *)describeLabel{
    if (!_describeLabel) {
        ...
        describeLabel.textColor = JhColor(221, 52, 60);
        describeLabel.font = [UIFont systemFontOfSize:13];
        describeLabel.textAlignment = NSTextAlignmentCenter;
        describeLabel.text = _describeStr;
        describeLabel.numberOfLines=2;
        describeLabel.frame =CGRectMake(10, CGRectGetMaxY(self.titleLabel.frame), Kwidth-20, kDescribeViewFrameH);
        if([_describeStr kr_isNotEmpty]){
            NSMutableAttributedString * attributedString = [[NSMutableAttributedString alloc] initWithString:_describeStr];
            NSMutableParagraphStyle * paragraphStyle = [[NSMutableParagraphStyle alloc] init];
            [paragraphStyle setLineSpacing:5];
            [attributedString addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, [_describeStr length])];
            [describeLabel setAttributedText:attributedString];
        }
        _describeLabel = describeLabel;
        
        [self.actionSheetView addSubview:self.describeLabel];
...
- (UICollectionView *)shareCollectionView{
    if (!_shareCollectionView) {
        ...
        CGRect Collectionframe =CGRectMake(0,CGRectGetMaxY(self.describeLabel.frame), viewWidth, viewHeight);
...
+ (void)showShareActionSheetWithTitle:(NSString *)title
    ...
    JhScrollActionSheetView *actionSheetView = [[JhScrollActionSheetView alloc]initWithTitle:title withdescribe:@"" shareDataArray:shareDataArray otherDataArray:nil];
...
+ (void)showShareActionSheetWithTitle:(NSString *)title
    ...
    JhScrollActionSheetView *actionSheetView = [[JhScrollActionSheetView alloc]initWithTitle:title withdescribe:content shareDataArray:shareDataArray otherDataArray:nil];
    actionSheetView.clickShareBlock = clickShareBlock;
    [actionSheetView show];
