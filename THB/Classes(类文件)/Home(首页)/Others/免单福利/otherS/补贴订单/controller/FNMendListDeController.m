...
@interface FNMendListDeController ()
@property(nonatomic,strong)NSMutableArray *titles;
@end

@implementation FNMendListDeController
...
- (void)viewDidLoad {
    ...
    self.titleScrollView.backgroundColor=RGB(240, 240, 240);
...
- (void)setupChildVc
{
    ...
    if(self.titles.count>0){
        for (int i = 0 ; i<self.titles.count; i++) {
            ...
            FNOrderMendDeController *VC = [[FNOrderMendDeController alloc] init];
...
- (FNRequestTool *)apiRequestCategory{
    ...
    return [FNRequestTool requestWithParams:params api:@"mod=appapi&act=appMiandanOrder&ctrl=cate" respondType:(ResponseTypeArray) modelType:@"XYTitleModel" success:^(id respondsObject) {
