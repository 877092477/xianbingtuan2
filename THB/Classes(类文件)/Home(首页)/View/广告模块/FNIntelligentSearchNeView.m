...
FNIntelligentSearchNeView *_SearchView = nil;

@interface FNIntelligentSearchNeView ()
...
@implementation FNIntelligentSearchNeView
...
+ (instancetype)shareInstance{
    if (_SearchView == nil) {
        ...
        dispatch_once(&onceToken, ^{
            _SearchView = [[FNIntelligentSearchNeView alloc]initWithFrame:FNKeyWindow.bounds];
        });
    }
    return _SearchView;
}
...
- (void)jm_setupViews{
    ...
    _seekJDBtn.tag=1;
    _searchBtn.cornerRadius=5;
    _searchBtn.titleLabel.font=kFONT13;
    [_searchBtn setTitle:@"" forState:UIControlStateNormal];
    [_searchBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_searchBtn addTarget:self action:@selector(storeTBAction) forControlEvents:(UIControlEventTouchUpInside)];
    ...
    [_BgView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(@((self.width-120)*1.2));
    }];
    ...
    [_ImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(@0);
        make.height.equalTo(@((self.width-120)*0.4));
    }];
    ...
    [_searchBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_commodityName.mas_bottom).offset(0);
        make.left.equalTo(_BgView.mas_left).offset(20);
        make.right.equalTo(_BgView.mas_right).offset(-20);
...
-(void)setFnIDstring:(NSString *)fnIDstring{
    ...
    if([fnIDstring kr_isNotEmpty]){
        [_searchBtn setTitle:@"跳详情" forState:UIControlStateNormal];
        
    }else{
        [_searchBtn setTitle:@"搜淘宝" forState:UIControlStateNormal];
    }
}
-(void)storeTBAction{
    NSString *content=@"";
...
- (void)showAnimation{
    _SearchView.BgView.transform = CGAffineTransformMakeScale(0.1, 0.1);
    [UIView animateWithDuration:0.25f animations:^{
        _SearchView.BgView.transform = CGAffineTransformMakeScale(1, 1);
    }];
}
...
+ (void)showWithModel:(id)model view:(UIView *)view storeTypeblock:(StoreTypeBlock)Typeblock{
    _SearchView = [FNIntelligentSearchNeView shareInstance];
//    _SearchView.storeTypeBlock=Typeblock;
    _SearchView.prasestring=model;
    [view addSubview:_SearchView];
    [_SearchView showAnimation];
}
+ (void)showWithContent:(NSString*)content view:(UIView *)view withfnID:(NSString*)fuID type: (FNIntelligentSearchType)type yhq_url: (NSString*)yhq_url storeTypeblock:(StoreTypeBlock)Typeblock{
    ...
    if (fuID == nil || [fuID isEqualToString:@""]) {
        _SearchView = [FNIntelligentSearchNeView shareInstance];
        _SearchView.storeTypeBlock=Typeblock;
        _SearchView.prasestring=content;
        _SearchView.fnIDstring = @"";
        _SearchView.model=nil;
        [view addSubview:_SearchView];
        [_SearchView showAnimation];
        return;
    }
    ...
    [FNRequestTool requestWithParams:params api:APIUrl respondType:(ResponseTypeDataKey) modelType:@"" success:^(id respondsObject) {
        ...
        if ([model.goods_title kr_isNotEmpty])
            _SearchView.prasestring=model.goods_title;
        if ([model.goods_title kr_isNotEmpty]) {
            _SearchView.fnIDstring=fuID;
            _SearchView.model=model;
        }
...
+ (void)dismiss {
    [[FNIntelligentSearchNeView shareInstance] removeFromSuperview];
}
